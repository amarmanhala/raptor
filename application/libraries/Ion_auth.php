...
class Ion_auth
{
		...
		if ( $this->ion_auth_model->forgotten_password($identity) )   //changed
		{
			...
			if ($user)
			{
				...
				else
				{
					...
					$message = '<h2>Hello '.$user->email.', </h2>';
					...
					$message  .= '<p>'. site_url('auth/resetpassword/'.$user->forgotten_password_code).'</p>';
					$message  .= '<p>Best Regards.</p>';
					$message  .= '<p>DCFM</p>';
					$message  .= '<hr/>';
					$message  .= '<p style="font-size:8px; color:#969AB3">Please do not reply to this message; it was sent from an unmonitored email address. This message is a service email related to your use of DCFM . For general inquiries or to request support with your account, please contact us for Support.</p>';
	...
	public function logout()
	{
		...
		$identity = $this->config->item('identity', 'ion_auth');
                $session_data = array($identity => '','raptor_identity' => '','raptor_user_id' => '','raptor_role'=> '','raptor_email' => '','raptor_customerid' => '', 'raptor_contactid' => '');
	...
	public function logged_in()
	{
		...
		return (bool) $this->session->userdata('raptor_identity');
	}
	...
	{
		$user_id = $this->session->userdata('raptor_user_id');
		if (!empty($user_id))
		{
			return $user_id;
		}

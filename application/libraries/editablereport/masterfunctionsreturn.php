<?php
class masterfunctionsreturn extends MY_Model 
{
    ...
    public function __construct()
    {
        ...
        foreach ($result->result_array() as $scf) {
        ...
        */
        ...
        //   $fieldid="customerid";
           ...
           case "radio":
           //show values as radio options;
           $tfields=$this->totalfieldarray($vt);
           $sqlx="select * from $vt";
           $fid=$vt."id";
           $fname=$vt."desc";
           $tfields=array($fid,$fname);
           $arrv=$this->infqueryarray_pair($sqlx,$tfields);
           //echo "<br> $vt : $arrx : $arrv";
           $GLOBALS["radiofield"][$fieldn]=$fieldn;
           if(isset($arrv)){
                foreach($arrv as $av){
        ...
        function makeselectoptions($sqlx,$dv,$fieldtofind){
          ...
          foreach ($result->result_array() as $vqd) {
        ...
        function makeidselectoptions($sqlx,$dv,$fieldtofind,$fieldid,$cao=true){
          ...
          $result = $this->db->query($sqlx);
          if(!$result) $erx=$this->db->_error_message();;
                $erx=$this->db->_error_number();
          if(!$result) $this->tfw("badq.txt","bad $sqlx error:$erx",true);
          if($cao){
        ...
        //echo "<br> found $fieldid $idval";
          ...
          $msx.="</SELECT>";
          #$this->tfw("msx.txt","$sqlx $msx $fieldid $fieldtofind",true);
          return $msx;
        ...
        //  $dfs.="<OPTION>".$dv."</OPTION>";
          ...
          foreach ($sfr->result_array() as $sfd) {
        ...
        function makedate_Form($fieldn,$fid="mods",$datastyle="formdata",$dv=null,$tix=null,$jsGuess="dateGuess",$postGuessCall=null,$clickx=null){
         ...
         //if($GLOBALS["pdate"]==2){
         $this->tfw("mdf$fieldn.txt",$x,true);
         return $x;
         //}else{
        ...
        */
        ...
        //echo $sqlx;
        $cresult = $this->db->query($sqlx);
        //if(!$cresult) error_message(sql_error());
        foreach ($cresult->result_array() as $cqd) {
        ...
        function includedfunctions($table){
         ...
         where tableid='$table'";
         $cresult = $this->db->query($sqlx);
         if(count($cresult->row_array())) {
             $qd = $cresult->row_array();
        ...
        function tsecurity($table,$el){
         ...
         and primid<>''
         ";
         $sresult = $this->db->query($sqlx);
         //if(!$sresult) error_message(sql_error());
         //$rc=mysql_num_rows($sresult);
         //if($rc>0){
                if(count($sresult->row_array())) {
                $sqd = $sresult->row_array();
        ...
        /**/
         ...
         //echo "<br> qqq $sqlx";
         $tresult = $this->db->query($sqlx);
         //if(!$tresult) error_message(sql_error());
         foreach ($tresult->result_array() as $cqd) {
        ...
        // echo "<br> qqq $sqlx";
         ...
         $px.=$this->inftable($tfields,$rd,$r,$eref,$detailidf,null,$newref,$vref);
        // $px.=$this->inftable($tfields,$rd,$r,$eref,$detailidf);
         return $px;
        }
        ...
        function infqueryarray_pair($sqlx,$tfields){
         ...
         if(!$tresult) $this->tfw("sqlPairerror.txt",sql_error(),true);
        ...
        function infqueryarray($sqlx,$tfields,$keyf=null,$tablen=null){
         $tresult = $this->db->query($sqlx);
          if(!$tresult){
                //$em=sql_error();
                $em=$this->db->_error_message();
                $errx=$sqlx." - ".$em;
                $this->tfw("errorview.txt",$errx,true);
          }

          if(isset($tablen)){
                if(!isset($tfields)){
                        $tfields=$this->key2val($this->totalfieldarray($tablen));
                }
         }
         if(!$tresult){
                $this->tfw("mqview.txt",$sqlx,false);
                return null;
         }
         $this->tfw("dmqview.txt",$sqlx,false);
        ...
        function infqueryarray_singlerow($sqlx,$tfields){
         //echo "qqq $sqlx";
         $tresult = $this->db->query($sqlx);
         if(count($tresult->row_array())) {
             $cqd = $tresult->row_array();
        ...
        function infquerydata($sqlx,$tfields,$usetips=false){
         //echo "ii $sqlx";
         $rd=$this->infqueryarray($sqlx,$tfields);
         $GLOBALS["iqda"]=$rd;
         $r=sizeof($rd);
         ...
         $px.=$this->inftable($tfields,$rd,$r,$eref,$detailidf,null,$newref,$vref,$usetips);
         return $px;
        }
        ...
        function totalfieldarray($table,$prefix=null){
         ...
         $this->tfw("tfax.txt",$sqlx,false);
         $result = $this->db->query($sqlx);
         foreach ($result->result_array() as $qd) {
        ...
        function ktfa($table,$prefix=null){
           $fa=$this->key2val($this->totalfieldarray($table,$prefix));
           return $fa;
        }

        function tfa($table,$prefix=null){
                $fa=$this->totalfieldarray($table,$prefix);
            return $fa;
        }

        function tinfo($t,$fn=null){
           $tf=$this->key2val(array_values($this->totalfieldarray("tablename")));
           if($fn!="") $tf=$this->kda($fn);
           $sqlx="select * from tablename where tableid='$t'";
           if( $t!=""){
                  $itfi=$this->iqa($sqlx,$tf,"fieldid");
           }
           return $itfi;
        }

        function tex($tn){
                if($this->tableExists($tn)) return true;
                return false;
        }
        function tableExists($tn){
                $q="show tables like '$tn'";
                if($this->qRowCount($q)>0) return true;
                return false;
        }
        ...
        function itfi($t){
           ...
           $sqlx="select * from tablename where tableid='$t'";
           if( $t!=""){
           $itfi=$this->infqueryarray_singlerow($sqlx,$tf);
           }
           return $itfi;
        }
        ...
        function inftable_rowarray($tableid,$idf=null,$idv=null){
         ...
         if(isset($idv)){
         $fields=$this->totalfieldarray($tableid);
         $sqlx="select * from $tableid
         where $idf='$idv'";
         //echo "<br>$sqlx";
         $result = $this->db->query($sqlx);
         if(!$result) $this->tfw("badifnra.txt",$sqlx,true);
         if(count($result->row_array())) {
             $vqd = $result->row_array();
        ...
        function infdataloader($table,$filterarr,$fieldsarr,$detailidf,$href,$editok,$tablevarray="table"){
         if(!isset($fieldsarr)){
         $fieldsarr=$this->key2val($this->totalfieldarray($table));
         ...
         switch($tablevarray){
          ...
          break;
          default:
          $px.=$this->inftable($fieldsarr,$rd,$r,$href,$detailidf,null,null,$editok);
          return $px;
          break;
         }
        }
         ...
         function field_valuesoffer($tableid,$fn,$fv,$wcount=false,$formname="fieldv"){
          ...
          $da=$this->infqueryarray_pair($sqlx,$tfields);
          if($wcount){
                  $jcall="onchange=\"document.subs.submit()\"";
          }
        ...
        function loadInputDates($tid){
         ...
         function distinct_array($table,$fieldid){
          ...
          $da=$this->infqueryarray_forkeylist($sqlx,$fieldid,$this->key2val(array($fieldid))) ;
          return $da;
         }
        ...
        function iqasrf($sqlx,$field){
                ...
                //$erx=$this->db->_error_number();
                #if(!$result) $this->tfw("badq.txt","$sqlx  err $erx",true);
                $result=$this->db->query($sqlx);
        ...
        function iqatr($table=null,$idf=null,$idv=null){
                ...
                #echo "<br>$qx";
                $this->tfw("iqa.txt",$qx,true);
                /*single row field query*/
                $result=$this->db->query($qx);
                $this->elog("iqat $qx","mastf 3119 ");
                if(count($result->row_array())) {
                    $cqd = $result->row_array();
                //if($cqd = mysql_fetch_array($result,MYSQL_ASSOC)){
                        foreach($cqd as $fn=>$fx) $fdx.="$fn = $fx";
                        $this->tfw("iqatt.txt",$fdx,true);
                        return $cqd;
                }
        }
        ...
        function iqasra($sqlx=null,$tf=null,$tid=null,$condx=null){
         ...
         if($sqlx!=""){
                ...
                if($tid!=""){
                        if(!isset($tf)) $tf=$this->tfa($tid);
                        if(is_array($tf)){
                                $fx=implode(",",$tf);
                                $qx="select $fx from $tid $condx";
                                $this->elog("iqa sra $qx","masterfuncret 3136");
                                $this->tfw("iqx$tid.txt",$qx,true);
                        }
                }else{
                        return;
                }
         }
         #$this->elog("iqa sra $qx","masterfuncret 3143");
         $tresult = $this->db->query($qx);
         $this->tfw("iquery.txt","q $qx",true);
         if(!tresult) $this->tfw("badquery.txt","q $qx",true);
         if(count($tresult->row_array())) {
             $cqd = $tresult->row_array();
        ...
        function aFV($da,$fn,$clean=false){
                ...
                if(isset($vals)) $vals=array_unique($vals);
                #echo $afx;
                #$this->elog("afv $fn $afx ","mfunc 3245");
                return $vals;
        }
        ...
        function dCV($table=null,$field=null,$aS=null){
                ...
                $this->tfw("dcq_prim.txt","$pk $q",true);

                if($pk=="") return;

                $this->tfw("dcq.txt",$q,true);
                $tf=$this->kda("fval,vcount");
                $da=$this->iqa($q,$tf);
                return $da;
        ...
        function aFVQ($sqlx,$fn){
                ...
                $da=$this->iqa($sqlx,$tf);
                $nda=$this->aFV($da,$fn);
                $kda=$this->key2val($nda);
                return $kda;
        }

        function iqa($sqlx,$tfields=null,$keyf=null,$tablen=null){
                $da=$this->infqueryarray($sqlx,$tfields,$keyf,$tablen);
                return $da;
        }
        ...
        function iqta($table,$aS){
         ...
         $tf=$this->key2val(array_keys($tfl));
         $tfx=implode(",",$tf);
        ...
        #echo $q;
                $this->tfw("qskx.txt",$q,true);
                $tresult = $this->db->query($q);
                $fa=$this->kda($fn);
                if($full){
                        $fa=$this->key2val($tfa);
                        $ta=$this->iqa($q,$fa);
        ...
        function formCruncher($str,$delim="!I!",$delim2="=",$lastInOk=false){
                ...
                #$this->tfw("fcr$sufx.txt","d1 $delim d2 $delim2 $str *** $px",true);

                if(strpos('more',$str)){
                        echo "alert('fcc $params')\n";
                        exit;
                }
                #$this->tfw("fcrunch$sufx.txt","s $str now $px",true);
                $tdx="";
                foreach($da as $dx){
                        $tdx.="~~ $dx";
                        ...
                        #$raw = (isset($ra[1]) ? $ra[1] : "");
                        $raw=$ra[1];
                        #$this->tfw("fc$rfn.txt","$raw : $rdv",true);
                ...
                #$this->tfw("fcrdx$sufx.txt","$tdx",true);
                #$this->tfw("nested.txt","$fcx $nestedx",true);
           return $newda;
        }
        ...
        function formCruncherMultiLine($str,$firstel="jline",$delim1="!I!",$delim2="="){
                ...
                $this->tfw("fcm.txt","px:$px str: $str fcm: $fcm",true);
           return $newda;
        }
        ...
        function tableData($table,$condx=null,$keyf=null){
                ...
                $tf=$this->key2val($this->tfa($table));
                $this->tfw("tdx.txt",$q,true);
                $da=$this->iqa($q,$tf,$keyf);
                return $da;
        }
        ...
        function dalist($da,$title="Listing",$colheads=array("item","value"),$classx=null){
                ...
                $ist=new divtable();
                $ist->noxl=1;
                $ist->showIt($this->key2val(array_values($colheads)),$row,$title);
        }
        ...
        public function kda($raw){
        $da=explode(",",$raw);
        $ka=$this->key2val($da);
        return $ka;
       }

       public function key2val($raw,$lcase=null){
        if(is_array($raw)){
               $fv=array_values($raw);
                foreach($fv as $v){

...
//        }
    ...
    public function loadScheduledServices() {
        ...
        { 
            ...
            if( $isSuccess )
            {
                ...
                $cweek = trim($this->input->get('cweek'));
                ...
                if(format_date($fromdate, 'm') == format_date($todate, 'm')) {
                    $month = array(
                        array(
                            'm' => format_date($fromdate, 'm'),
                            'y' => format_date($fromdate, 'Y'),
                        )
                    );
                } else {
                    $month = array(
                        ...
                        array(
                            'm' => format_date($todate, 'm'),
                            'y' => format_date($todate, 'Y')  
                        )
                    );
                }
                ...
                $columns = $this->createScheduledServicesColumns($month, $year, $show, $cweek, $fromdate, $todate);
                ...
                if(count($labelids) > 0) {
                    ...
                    $data = $this->formatScheduledServicesData($columns, $data, $siteData, $year, $show, $fromdate, $todate);
                }
            ...
            foreach ($columns as $key2 => $value) {  
                ...
                if((bool)$value['d']) {
                    if($cweek == '') {
                        ...
                        foreach($value['days1'] as $w) {
                            $val = explode(' ', $w);
                            $this->excel->getActiveSheet()->setCellValue($colH.$rowNo, $row1['ex_d_' . $value['monthNumeric'] . '_' . $val[0]]);
                            $colH++;
                        }
                    }
                }
    ...
    * @return array 
    */
    private function createScheduledServicesColumns($month, $year, $show, $cweek, $fromdate, $todate) {

        $columns = array();
        //$year = 2016;
        $fyear = format_date($fromdate, 'Y');
        $tyear = format_date($todate, 'Y');

        //$show = 'week';
        //$show = 'month';
        for($i = 1; $i <= 12; $i++) {
            $filterMonths = getMonthsInRange($fromdate, $todate);
            $tMonths = array();
            ...
            if(in_array($i, $tMonths)) {
                ...
                foreach($weekNumbersArr as $w) {
                    ...
                    $weeks[] = array(
                        ...
                        'day' => ordinal(date("j", strtotime($weekStartEndDateArr[0]))),
            ...
            if($show == 'week') {
                ...
                if(in_array($i, $tMonths)) {
                    $columns[] = array(
                        'month' => date('F', mktime(0, 0, 0, $i, 10, $year)),
                        'monthNumeric' => $i,
                        'weeks' => $weeks,
                        'm' => 0,
                        'w' => 1,
                ...
                if(in_array($i, $tMonths)) {
                    $columns[] = array(
                        'month' => date('F', mktime(0, 0, 0, $i, 10, $year)),
                        'monthNumeric' => $i,
                        'start' => date('Y-m-01', mktime(0, 0, 0, $i, 10, $year)),
                        'end' => date('Y-m-t', mktime(0, 0, 0, $i, 10, $year)),
                        'm' => 1,
                        'w' => 0,
                        'd' => 0
                    );
        ...
        if($show == 'day') {
            foreach($month as $m) {
                ...
                if($cweek == '') {
                    ...
                    $filterDates = getDatesFromRange($start, $end, $step = '+1 day', $output_format = 'Y-m-d');
                    $startDay = date('d', strtotime($fromdate));
                    $endDay = date('t', strtotime($fromdate));
                    
                    foreach($filterDates as $key5=>$value5) {
                        $month5 = date('m', strtotime($value5));
                        if($m == $month5) {
                        } else {
                            unset($filterDates[$key5]);
                        }
                    }

                    $days = array_map(function($num) {
                        $day = ordinal(date('j', strtotime($num)));
                        return $day;
                    }, $filterDates);
                } else {
                    $start = date('Y-m-d', strtotime($fromdate));
                    ...
                    $filterDates = getDatesFromRange($start, $end, $step = '+1 day', $output_format = 'Y-m-d');
                    $startDay = date('d', strtotime($fromdate));
                    $endDay = date('d', strtotime($todate));
                    $dayArr = range($startDay, $endDay);
                    $days = array_map("ordinal", $dayArr);
                    $days1 = array_map(function($num) {
                        $day = ordinal(date('j', strtotime($num)));
                        $r = $day.' '.date('D', strtotime($num));
                        return $r;
                    }, $filterDates);
                }
    ...
    * @return array 
    */
    private function formatScheduledServicesData($columns, $data, $siteData, $year, $show, $fromdate, $todate) {
        ...
        foreach ($data as $key => $siterow) {
                    ...
                    foreach($c['weeks'] as $w) {
                        ...
                        foreach($siteData as $service) {
                            ...
                            if($labelid == $service['labelid']) {
                                ...
                                $tWeeks = array_unique($tWeeks);
                                
                                                                
                                $fStartTime = strtotime($fromdate);
                                $tEndTime = strtotime($todate);
                                
                                $filterWeeks = array();
                                while ($fStartTime < $tEndTime) {  
                                    $filterWeeks[] = date('W', $fStartTime); 
                                    //$fStartTime += strtotime('+1 week', 0);
                                    $fStartTime += strtotime('+1 day', 0);
                                }
                                $filterWeeks = array_unique($filterWeeks);

                                foreach($tWeeks as $tw) {
                                    if($tw == $w['weekNumber'] && in_array($w['weekNumber'], $filterWeeks)) {
                                    //if($tw == $w['weekNumber']) {
                                        $icon = $service['icon'];
            ...
            if($show == 'month') {
                foreach($columns as $k=>$c) {
                    ...
                    foreach($siteData as $service) {
                        if($labelid == $service['labelid']) {
                            ...
                            foreach($tWeeks as $tw) {
                                ...
                                if($tw == $c['monthNumeric'] && in_array($tw, $tMonths)) {
                                    $icon = $service['icon'];
                                    $thisicon = "<i class='fa $icon'></i>";
                    ...
                    $data[$key]['mon_' . ($c['monthNumeric'])] = $showIcon;
                    //for excel
                    $data[$key]['ex_mon_' . ($c['monthNumeric'])] = implode(PHP_EOL, $statusName);
                }
            }
                    ...
                    foreach($c['days'] as $w) {
                        ...
                        foreach($siteData as $service) {
                            if($labelid == $service['labelid']) {
                                ...
                                $filterDates = getDatesFromRange($fromdate, $todate, $step = '+1 day', $output_format = 'Y-m-d');
                                $filterDates = array_map(function($num){return strtotime($num);}, $filterDates);
                                foreach($tWeeks as $tw) {
                                    
                                    if(strtotime($tw) == strtotime($cDay) && in_array(strtotime($tw), $filterDates)) {

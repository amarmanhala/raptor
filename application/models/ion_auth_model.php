...
class Ion_auth_model extends CI_Model
{
	...
	public function hash_password_db($id, $password, $use_sha1_override=FALSE)
	{
		...
		$query = $this->db->select('raptorpassword as raptorpassword, salt')
		...
		if ($use_sha1_override === FALSE && $this->hash_method == 'bcrypt')
		{
			...
			if ($this->bcrypt->verify($password, $hash_password_db->raptorpassword))
			{
                            return TRUE;
			}
			return FALSE;
		...
		{
			$salt = substr($hash_password_db->raptorpassword, 0, $this->salt_length);

			$db_password =  $salt . substr(sha1($salt . $password), 0, -$this->salt_length);
		}

		if($db_password == $hash_password_db->raptorpassword)
		{
			return TRUE;
		}
		else
	...
	public function activate($id, $code = false)
	{
		...
		if ($code !== FALSE)
		{
			...
			$this->trigger_events('extra_where');
			$this->db->update($this->tables['users'], $data, array('contactid' => $id));
		}
		else
		{
			$data = array(
...
*/
	...
	public function reset_password($identity, $new) {
		...
		$query = $this->db->select('contactid, raptorpassword, salt')
	...
	public function change_password($identity, $old, $new)
	{
		...
		$old_password_matches = $this->hash_password_db($user->contactid, $old);
		...
		if ($old_password_matches === TRUE)
		{
			...
			$data = array(
			    'raptorpassword' => $hashed_new_password,
			    'remember_code' => NULL,
			);
	...
	public function forgotten_password_complete($code, $salt=FALSE)
	{
		...
		if ($profile) {
			...
			$data = array(
			    'raptorpassword'               => $this->hash_password($password, $salt),
			    'forgotten_password_code' => NULL,
			    'active'                 => 1
			 );
	...
	public function register($username, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$this->trigger_events('extra_set');
                //$this->db->insert('customer', $additional_data);
		//$cid = $this->db->insert_id();
		//$user_data['customerid']=$cid;
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', contactid, customerid, contactid as id, raptorpassword, email, role, iscpmaster, active, last_login, israptoradmin, cp_invitesendtime')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		                  ->get($this->tables['users']); 
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE)
			{
                            ...
                            if ($user->active == 0) {
                                $loginLog['success'] = 0;
                                $this->create_login_logs($loginLog);
                                $this->trigger_events('post_login_unsuccessful');
                            ...
                            if ($user->cp_invitesendtime != NULL && $user->cp_invitesendtime != "0000-00-00 00:00:00") {
                                $cp_invitesendtime =  strtotime($user->cp_invitesendtime);
                                $currentdtime =  time();
                                 $diff = ($currentdtime - $cp_invitesendtime)/60;
                                $raptor_welcome_timeout = (int)$this->config->item('raptor_welcome_timeout');
                                if ($raptor_welcome_timeout == 0) {
                                    $raptor_welcome_timeout = 60;
                                } 
                                  $raptor_welcome_timeout;
                               
                                if ($diff > $raptor_welcome_timeout) {
                                    $loginLog['success'] = 0;
                                    $this->create_login_logs($loginLog);
                                    $this->trigger_events('post_login_unsuccessful');
                                    $this->set_error('This password has expired. Please contact your administrator and request another invitation.');
                                    return FALSE;
                                } 
                                 
                                $this->clear_invitesendtime($user->id);
                            }
                            
                            $query1 = $this->db->select('companyname, cpstatus')
                                ...
                                if ($query1->num_rows() == 0)
                                {
                                    ...
                                    $this->create_login_logs($loginLog);
                                    $this->trigger_events('post_login_unsuccessful');
                                    $this->set_error('Your company does not currently have access to the client portal. Contact us on info@itglobal-systems.com');
                                    return FALSE;
                                }
                            ...
                            $this->update_Customer_login($user->customerid);
		...
		if ($this->config->item('track_login_attempts', 'ion_auth')) {
			...
			if ($max_attempts > 0) {
				...
				if($max_attempts>$attempts){
					$this->set_error($max_attempts-$attempts.' login attempts remain.');
				}
	 ...
	 **/
	public function create_login_logs($logsData) {
		 
            $ip_address = $this->_prepare_ip($this->input->ip_address());
            $logsData['ipaddress'] = $ip_address;
            return $this->db->insert('loginlog', $logsData);
	...
	public function users($groups = NULL)
	{
		...
		else
		{
			...
			$this->db->select(array(
			    $this->tables['users'].'.*',
			    $this->tables['users'].'.contactid as id',
			    $this->tables['users'].'.contactid as user_id'
			));
		}
		...
		if (isset($groups))
		{
			...
			if (isset($groups) && !empty($groups))
			{
				...
				$this->db->join(
				    ...
				    $this->tables['users_groups'].'.'.$this->join['users'].'='.$this->tables['users'].'.contactid',
				    'inner'
				);
	...
	public function user($id = NULL)
	{
		...
		$id || $id = $this->session->userdata('raptor_user_id');

		$this->limit(1);
		$this->where($this->tables['users'].'.contactid', $id);
	...
	public function add_to_group($group_id, $user_id=false)
	{
		...
		//if no id was passed use the current users id
		$user_id || $user_id = $this->session->userdata('raptor_user_id');
	...
	public function update($id, array $data)
	{
		...
		$this->trigger_events('extra_where');
		$this->db->update($this->tables['users'], $data, array('contactid' => $user->id));
	...
	**/
	public function delete_user($id)
	{
		...
		$this->db->delete($this->tables['users'], array('contactid' => $id));
	 ...
	 * @author Ben Edmunds
	 **/
	public function clear_invitesendtime($id)
	{
            $this->trigger_events('clear_invitesendtime');

            $this->load->helper('date');


            $this->db->update($this->tables['users'], array('cp_invitesendtime' => NULL), array('contactid' => $id));
	...
	public function update_last_login($id)
	{
		...
		$this->db->update($this->tables['users'], array('last_login' => time()), array('contactid' => $id));
	 ...
	 * @author Ben Edmunds
	 **/
	public function update_Customer_login($customerid)
	{
		 
  

		$this->db->update('customer', array('lastcplogin' => date('Y-m-d H:i:s')), array('customerid' => $customerid));
	...
	public function set_session($user)
	{
		...
		$this->trigger_events('pre_set_session');
                $role='';
                if($user->iscpmaster == NULL || $user->iscpmaster!=1)
                {
		...
		$session_data = array(
		    'raptor_identity'            => $user->{$this->identity_column},
                    'raptor_israptoradmin'       => $user->israptoradmin,
                    'raptor_user_id'             => $user->contactid,
                    'raptor_role'                => $role,
		    'raptor_email'               => $user->email,
		    'raptor_customerid'          => $user->customerid, //everyone likes to overwrite id so we'll use user_id
		    'raptor_contactid'           => $user->contactid, //everyone likes to overwrite id so we'll use user_id
	...
	public function remember_user($id)
	{
		...
		$this->db->update($this->tables['users'], array('remember_code' => $salt), array('contactid' => $id));
	...
	public function login_remembered_user()
	{
            ...
            $query = $this->db->select($this->identity_column . ', contactid, customerid, contactid as id, raptorpassword, email, role,iscpmaster, active, last_login, israptoradmin')
            ...
            if ($query->num_rows() == 1)
            {
                    ...
                    $ip_address = $this->_prepare_ip($this->input->ip_address());
                    $loginLog = array(
                        'username'  => get_cookie($this->config->item('identity_cookie_name', 'ion_auth')),
                        'password'  => get_cookie($this->config->item('remember_cookie_name', 'ion_auth')),
                        'login'     => date('Y-m-d H:i:s'),
                        'ipaddress' => $ip_address,
                        'userid'    => $user->id
                    );
	...
	**/
	...
	public function update_password($identity, $new)
	{
                ...
                $successfully_changed_password_in_db = $this->db->update($this->tables['users'], $data, array('contactid' => $identity));

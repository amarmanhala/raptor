    ...
    app.controller('BudgetCtrl', [
       ...
       $scope.gridOptions = {
         ...
         columnDefs: [
            { 
                ...
                enableSorting: false,
                width: 70,
                cellTemplate: '<div class="ui-grid-cell-contents" title="Detail" ng-if="!row.entity.detail" ></div><div class="ui-grid-cell-contents" title="Documents" ng-if="row.entity.detail" ><button type="button" data-id="{{row.entity.labelid}}"  data-site="{{row.entity.siteline1}} {{row.entity.siteline2}} {{row.entity.sitesuburb}} {{row.entity.sitestate}}" class="btn btn-flat btn-default btn-xs btn_budget_detail"   >Detail</button></div>'
            },
            { 
       ...
       $scope.changeText = function() {
        ...
        $scope.exportToExcel=function(){
            ...
            window.open(base_url+'budgets/downloadexcelbysite?'+$.param($scope.filterOptions));

        };
        $scope.exportImportTemplate=function(){
            var url = base_url + 'budgets/downloadimportfilebysite/'+$scope.filterOptions.year.replace('-', '/');
            window.open(url);
       ...
       var getPage = function() {
            ...
            $('#BudgetCtrl .overlay').show();
             $http.get(base_url+'budgets/loadbudgetsbysite?'+ qstring ).success(function (data) {
...
$( document ).ready(function() {
    ...
    $(document).on('click', '.btn_budget_detail', function() {
        ...
        $.get( base_url + 'budgets/loadbudgetdetailbysite/'+recordid+'/'+year, { 'get':1}, function( data ) {
    ...
    if (typeof $.fn.validate === "function") {
		...
		submitHandler: function() {
			...
			$.post( base_url+"budgets/createannualbudgetbysite/q", $("#createbudget_form").serialize(), function( data ) {
                            $("#createbudget #budgetbtnsave").button('reset');
				if(data.success) {
                                            ...
                                            bootbox.dialog({
                                                    ...
                                                    buttons:
                                                    {
                                                            "success" :
                                                             {
                                                                    ...
                                                                    "callback": function() {
                                                                        $.post( base_url+"budgets/updateannualbudgetbysite/q", $("#createbudget_form").serialize(), function( data2 ) {
                                                                            if(data2.success) {
                                                                                $("#createbudget").modal('hide');
                                                                                $( "#BudgetCtrl .btn-refresh" ).click();
                                                                            }
                                                            ...
                                                            "click" :
                                                            {
                                                                    ...
                                                                    "callback": function() {
                                                                         $.post( base_url+"budgets/addannualbudgetbysite/q", $("#createbudget_form").serialize(), function( data2 ) {
                                                                            if(data2.success) {
                                                                                $("#createbudget").modal('hide');
                                                                                $( "#BudgetCtrl .btn-refresh" ).click();
                                                                            } 
